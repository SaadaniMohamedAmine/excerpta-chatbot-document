...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "flex h-full shrink-0 flex-col border-r border-border bg-surface py-4 transition-[width] duration-150",
        collapsed ? "w-16" : "w-64",
    ...
    >
      ...
      <div className={cn("mb-4 px-3", collapsed && "px-2")}>
        <Link
          href="/documents"
          className={cn(
            "flex h-10 items-center justify-center gap-2 rounded-md bg-primary font-sans text-sm font-medium text-white transition-colors hover:bg-primary/90",
      ...
      <nav className="flex flex-col gap-1 px-3">
        {!collapsed && (
          <span className="mb-1 px-2 font-sans text-xs font-medium uppercase tracking-wide text-text-secondary">
            Workspace
          </span>
        )}
      ...
      {/* Collapse toggle */}
      <div className={cn("mb-2 px-3", collapsed && "px-2")}>
        <button
          type="button"
          ...
          className={cn(
            "flex h-9 items-center gap-2 rounded-md text-text-secondary transition-colors hover:bg-background hover:text-text-primary",
            collapsed ? "w-full justify-center" : "w-full px-2"
          )}
        >
          <SidebarSimple size={18} weight="regular" />
          {!collapsed && <span className="font-sans text-sm">Collapse</span>}
        </button>
      </div>

      {/* Profile block */}
      <div className={cn("border-t border-border px-3 pt-3", collapsed && "px-2")}>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              className={cn(
                "flex items-center gap-2 rounded-md py-1.5 text-left transition-colors hover:bg-background",
...
function SidebarNavItem({
  ...
  return (
    <Link
      ...
      title={item.label}
      className={cn(
        "relative flex h-10 items-center gap-2.5 rounded-md text-sm font-medium text-text-secondary transition-colors hover:bg-background hover:text-text-primary",
